...
def test_no_whole_document_scan_tests_an_attribute_on_every_element(scans, page):
    # The declared readers' scans: the guesses of what the page shows add
    # their own (visible.py), measured apart.
    extract((FIXTURES / page).read_bytes(), url="https://shop.example/p", visible=False)
    assert scans
