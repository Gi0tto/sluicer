...
result back. It sits behind the ``markdown`` extra, so a reader who never asks
for markdown does not carry it. ``full=True`` writes the whole page instead,
menus and footers included (``sluicer.markdown_writer``), and needs only lxml.
``read_markdown`` says which of the two the text is.
"""
